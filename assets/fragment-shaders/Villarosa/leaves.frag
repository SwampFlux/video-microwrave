...
float random (vec2 st) { 
    return fract(sin(dot(st.xy, vec2(12.9898,78.233)) + u_analog0 ) * 43758.5453123);
}
...
float circle(in vec2 _st, in vec2 pos, in float _radius){
    ...
    float circEdge  =  20.0 / u_resolution.x;
...
void main(void)
{
    ...
    float rndTime = randomRange(vec2(floor(u_time/flipTime), 79834.345),3.,16.);
